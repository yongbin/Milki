...
sub test_email {
    ...
    for my $header ( sort keys %{$headers} ) {
        ...
        if ( ref $expect ) {
            like( scalar $email->header($header),
                $expect, "$header matches regex" );
        }
        else {
            is( scalar $email->header($header),
                $expect, "$header header is correct" );
        }
    }
    ...
    like( $html->body(), $html_re, 'html body matches regex' );
    ...
    like( $text->body, $text_re, 'plain text body matches regex' );
}
