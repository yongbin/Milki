...
use DateTime;
use Milki::Localize::Format::Gettext;

my $formatter = Milki::Localize::Format::Gettext->new();

is( $formatter->html( 'en', ['<foo>'] ), '&lt;foo&gt;', 'html escaping' );

is( $formatter->quant( 'en', [0, 'dog', 'dogs'] ), '0 dogs', 'quant for 0' );

is( $formatter->quant( 'en', [0, 'dog', 'dogs', 'no dogs'] ),
    'no dogs', 'quant for 0 with third form' );

is( $formatter->quant( 'en', [1, 'dog', 'dogs'] ), '1 dog', 'quant for 1' );

is( $formatter->quant( 'en', [42, 'dog', 'dogs'] ), '42 dogs',
    'quant for 42' );
...

{
    ...
    is(
        $formatter->on_date( 'en_US', [DateTime->new( year => 2009 )], ),
        'on Jan 1, 2009',
        'on_date for date in different year than today'
    );

    is(
        $formatter->on_date( 'en_US', [DateTime->new( year => 2010 )], ),
        'on Jan 1',
