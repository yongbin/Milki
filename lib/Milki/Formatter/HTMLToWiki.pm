package Milki::Formatter::HTMLToWiki;
...
use Milki::Formatter::HTMLToWiki::Table;
use Milki::Types qw( Maybe Str ArrayRef PosOrZeroInt );
...
has _stream => ( is => 'rw', isa => OutputStream, init_arg => undef, );
...
has _indent_level => (
    ...
    isa      => PosOrZeroInt,
    default  => 0,
    handles  => { _inc_indent_level => 'inc', _dec_indent_level => 'dec', },
    init_arg => undef,
);

has _bullet => ( is => 'rw', isa => Maybe [Str], );

has _current_href => ( is => 'rw', isa => Maybe [Str], );

has _table => (
    is       => 'rw',
    isa      => 'Milki::Formatter::HTMLToWiki::Table',
...
sub _handle_events_from_tree {
    ...
    for my $node ( $tree->content_list() ) {
        ...
        $self->$start($node) if $self->can($start);

        $self->_handle_node($node);

        $self->_handle_events_from_tree($node);

        $self->$end($node) if $self->can($end);
    }
}
...
sub _handle_a_as_wiki_link {
    ...
    if ( $href =~ m{^/wiki/([^/]+)/page/([^/]+)} ) {
        $wiki         = $1;
        $thing        = Milki::Schema::Page->URIPathToTitle($2);
        $default_text = $thing;
    }
        ...
        my $file = Milki::Schema::File->new( file_id => $2 );
...
sub _handle_table {
    ...
    my $table      = Milki::Formatter::HTMLToWiki::Table->new();
    my $old_stream = $self->_stream();
