package Milki::Formatter::HTMLToWiki::Table;
...
use Milki::Formatter::HTMLToWiki::Table::Cell;
use Milki::Types qw( Str ArrayRef Int Bool Maybe );
...
my $row = subtype as ArrayRef ['Milki::Formatter::HTMLToWiki::Table::Cell'];
...
has _thead_rows => (
    ...
    isa      => ArrayRef [$row],
    default  => sub { [] },
    handles  => { _add_thead_row => 'push', _has_thead => 'count', },
    init_arg => undef,
);

has _in_thead => ( is => 'rw', isa => Bool, default => 0, init_arg => undef, );
...
has _tbodies => (
    ...
    default => sub { [] },
    handles  => { _add_tbody => 'push', _has_tbodies => 'count', },
    init_arg => undef,
);
...
has _current_tbody => (
    ...
    isa      => ArrayRef [$row],
    default  => sub { [] },
    handles  => { _add_tbody_row => 'push', _reset_current_tbody => 'clear', },
    init_arg => undef,
);
...
has _current_cell => (
    is       => 'rw',
    isa      => 'Milki::Formatter::HTMLToWiki::Table::Cell',
    init_arg => undef,
);
...
sub _start_cell {
    ...
    $self->_set_current_cell(
        Milki::Formatter::HTMLToWiki::Table::Cell->new(
...
sub as_markdown {
    ...
    for my $tbody (@tbodies) {
        ...
        push @rows, q{} if $tbody ne $tbodies[-1];
    }

    push @rows, $divider if $self->_has_thead();
