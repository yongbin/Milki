...
use Milki::Types qw( Int Str Bool );
...
with 'Milki::Role::Schema::DataValidator' => {
    steps => [
        '_has_password_or_openid_uri',        '_email_address_is_unique',
        '_normalize_and_validate_openid_uri', '_openid_uri_is_unique',
    ],
};
...
around insert => sub {
    ...
    if ( delete $p{requires_activation} ) {
        $p{activation_key}
            = sha1_hex( $p{email_address}, Milki::Config->new()->secret() );
    }
    ...
    $p{created_by_user_id} = $p{user}->user_id() if $p{user};
...
sub _system_log_values_for_update {
    ...
    my $msg;
    if ( exists $p{is_disabled} && $p{is_disabled} && !$self->is_disabled() ) {
    ...
    return ( message => $msg, data_blob => \%blob, );
}
...
sub _normalize_and_validate_openid_uri {
    ...
    unless ( defined $uri->scheme() && $uri->scheme() =~ /^https?/ ) {
        return {
            field   => 'openid_uri',
...
sub domain {
    ...
    return $wiki ? $wiki->domain() : Milki::Schema::Domain->DefaultDomain();
}
...
sub _CreateSpecialUser {
    ...
    local $Milki::Role::Schema::SystemLogger::SkipLog = 1 unless $creator;
...
sub has_permission_in_wiki {
    ...
    my ( $wiki, $perm ) = validated_list(
        \@_,
        wiki       => { isa => 'Milki::Schema::Wiki' },
        permission => { isa => 'Milki::Schema::Permission' },
    );
...
sub is_wiki_member {
    my $self = shift;
    my ($wiki) = pos_validated_list( \@_, { isa => 'Milki::Schema::Wiki' } );
...
sub role_in_wiki {
    ...
    $role_name ||= $self->is_guest() ? 'Guest' : 'Authenticated';

    return Milki::Schema::Role->$role_name();
}
...
sub _role_name_in_wiki {
    ...
    my $row
        = $dbh->selectrow_arrayref( $select->sql($dbh), {}, $wiki->wiki_id(),
        $self->user_id(), );
...
sub _MemberWikiSelectBase {
    ...
    my $authed = Milki::Schema::Role->Authenticated();
...
sub _BuildAllWikiCountSelect {
    ...
    $select->select($count)->from( $Schema->table('Wiki') )
        ->where( $Schema->table('Wiki')->column('wiki_id'),
        'IN', $explicit_wiki_select )->where('or')
        ->where( $Schema->table('Wiki')->column('wiki_id'),
...
sub _BuildAllWikiSelect {
    ...
    $union->union( $explicit_wiki_select, $implicit_wiki_select )->order_by(
        Fey::Literal::Term->new($is_explicit_idx), 'DESC',
        Fey::Literal::Term->new($title_idx),       'ASC',
    );
...
sub _ImplicitWikiSelectBase {
    ...
    my $explicit = Milki::Schema->SQLFactoryClass()->new_select();
...
sub _BuildRecentlyViewedPagesSelect {
    ...
    my $max_datetime = Milki::Schema->SQLFactoryClass()->new_select();
    $max_datetime->select($max_func)->from($page_view_t)
        ->where( $page_view_t->column('page_id'),
        '=', $page_t->column('page_id') );

    my $viewed_select = Milki::Schema->SQLFactoryClass()->new_select();
    $viewed_select->select($page_t)->from( $page_t, $page_view_t )
        ->where( $page_view_t->column('user_id'),
        '=', Fey::Placeholder->new() )
...
sub _send_email {
    ...
    my ( $wiki, $sender, $message, $template ) = validated_list(
        \@_,
        wiki     => { isa => 'Milki::Schema::Wiki', optional => 1 },
        sender   => { isa => 'Milki::Schema::User' },
    ...
    my $subject = $wiki
        ...
        $wiki->title(), $wiki->domain()->web_hostname(),
        )
        : loc( 'Activate your user account on the %1 server',
        $self->domain()->web_hostname() );

    my $from
        = Email::Address->new( $sender->best_name(), $sender->email_address() )
        ->format();

    my $to = Email::Address->new( $self->best_name(), $self->email_address(), )
        ->format();
...
sub _BuildAllUserSelect {
    ...
    $select->select($user_t)->from($user_t)->order_by($order_by);
