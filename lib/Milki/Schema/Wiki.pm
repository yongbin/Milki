...
use Milki::Schema::TextSearchResult;
use Milki::Schema::UserWikiRole;
use Milki::Schema::WantedPage;
use Milki::Schema::WikiRolePermission;
use Milki::Types qw( Bool HashRef Int Str ValidPermissionType );
...
with 'Milki::Role::Schema::SystemLogger' =>
    { methods => ['insert', 'delete'] };

with 'Milki::Role::Schema::DataValidator' => { steps =>
        ['_title_is_unique', '_title_is_valid', '_short_name_is_unique',], };
...
For more information about wikis in general and Milki in particular, click on the Help link in the upper right.
...
around insert => sub {
    ...
    $p{account_id} = Milki::Schema::Account->DefaultAccount()->account_id();
...
sub _system_log_values_for_insert {
    ...
    return ( message => $msg, data_blob => \%p, );
}
...
sub _title_is_unique {
    ...
    return {
        field => 'title',
        message =>
            loc('The title you provided is already in use by another wiki.'),
    };
}
...
sub add_user {
    ...
    my ( $user, $role ) = validated_list(
        ...
        role => {
            ...
            default => Milki::Schema::Role->Member(),
        },
    );
    ...
    my $uwr = Milki::Schema::UserWikiRole->new(
    ...
    if ($uwr) {
        ...
        Milki::Schema::UserWikiRole->insert(
            user_id => $user->user_id(),
            wiki_id => $self->wiki_id(),
...
sub remove_user {
    my $self = shift;
    my ($user)
        = validated_list( \@_, user => { isa => 'Milki::Schema::User' }, );
...
sub _build_permissions {
    ...
    for my $row (
        @{
            $dbh->selectall_arrayref( $select->sql($dbh), {},
                $select->bind_params() )
        }
        )
    {
...

{
    ...
    my $Delete = Milki::Schema->SQLFactoryClass()->new_delete();
    ...
    sub set_permissions {
        ...
        for my $role_name ( keys %{$set} ) {
            my $role = Milki::Schema::Role->$role_name();

            for my $perm_name ( @{ $set->{$role_name} } ) {
                my $perm = Milki::Schema::Permission->$perm_name();
        ...
        my $dbh = Milki::Schema->DBIManager()->source_for_sql($Delete)->dbh();
        my $trans = sub {
            $dbh->do( $Delete->sql($dbh), {}, $self->wiki_id() );
            Milki::Schema::WikiRolePermission->insert_many(@inserts);
        };
...
sub _DistinctRevisionCountSelect {
    ...
    my $count
        = Fey::Literal::Function->new( 'COUNT',
        $page_revision_t->column('page_id'),
        );

    my $count_select = Milki::Schema->SQLFactoryClass()->new_select();
    $count_select->select($count)->from( $page_t, $page_revision_t )
        ->where( $page_t->column('wiki_id'), '=', Fey::Placeholder->new() )
...
sub revisions {
    ...
    return Fey::Object::Iterator::FromSelect->new(
        classes => ['Milki::Schema::Page', 'Milki::Schema::PageRevision'],
        select  => $select,
        dbh => Milki::Schema->DBIManager()->source_for_sql($select)->dbh(),
...
sub _BuildRecentChangesSelect {
    ...
    my $pages_select = Milki::Schema->SQLFactoryClass()->new_select();
...
sub _BuildDistinctRecentChangesSelect {
    ...
    $pages_select->select( $page_t, $Schema->table('PageRevision') )
        ...
        '=', $max_revision )
        ->order_by(
        $Schema->table('PageRevision')->column('creation_datetime'),
        'DESC', $page_t->column('title'), 'ASC', );
...
sub _PageCountSelect {
    ...
    my $pages_select = Milki::Schema->SQLFactoryClass()->new_select();
    $pages_select->select($count)->from($page_t)
        ->where( $page_t->column('wiki_id'), '=', Fey::Placeholder->new() );
}
...
sub _OrphanedPageCountSelect {
    ...
    my $linked_pages = Milki::Schema->SQLFactoryClass()->new_select();
...
sub _BuildOrphanedPagesSelect {
    ...
    my $pages_select = Milki::Schema->SQLFactoryClass()->new_select();
    $pages_select->select($page_t)->from($page_t)
        ->where( $page_t->column('wiki_id'), '=', Fey::Placeholder->new() )
        ->and( $page_t->column('title'),   '!=',     Fey::Placeholder->new() )
        ->and( $page_t->column('page_id'), 'NOT IN', $linked_pages )
        ->order_by( $page_t->column('title'), 'ASC', );
...
sub _WantedPageCountSelect {
    ...
    my $wanted_select = Milki::Schema->SQLFactoryClass()->new_select();

    $wanted_select->select($count)->from($pending_page_link_t)
...
sub wanted_pages {
    ...
    return Fey::Object::Iterator::FromSelect->new(
        classes => ['Milki::Schema::WantedPage'],
        select  => $select,
        dbh     => Milki::Schema->DBIManager()->source_for_sql($select)->dbh(),
        bind_params   => [$self->wiki_id()],
        attribute_map => {
            0 => {
                class     => 'Milki::Schema::WantedPage',
                attribute => 'title',
            },
            1 => {
                class     => 'Milki::Schema::WantedPage',
...
sub _BuildWantedPagesSelect {
    ...
    my $wanted_select = Milki::Schema->SQLFactoryClass()->new_select();
    $wanted_select->select(
        $pending_page_link_t->columns( 'to_page_title', 'to_wiki_id' ),
        $count )->from($pending_page_link_t)
        ->where( $pending_page_link_t->column('to_wiki_id'),
        '=', Fey::Placeholder->new() )
        ->group_by(
        $pending_page_link_t->columns( 'to_page_title', 'to_wiki_id' ) )
...
sub recently_viewed_pages {
    ...
    return Fey::Object::Iterator::FromSelect->new(
        classes => ['Milki::Schema::Page'],
        select  => $select,
        dbh     => Milki::Schema->DBIManager()->source_for_sql($select)->dbh(),
        bind_params =>
            [$self->wiki_id(), DateTime::Format::Pg->format_datetime($cutoff)],
    );
}
...
sub _BuildRecentlyViewedPagesSelect {
    ...
    my $viewed_select = Milki::Schema->SQLFactoryClass()->new_select();
    $viewed_select->select( $page_t, $count )->from( $page_t, $page_view_t )
        ->where( $page_t->column('wiki_id'), '=', Fey::Placeholder->new() )
        ->and( $page_view_t->column('view_datetime'),
        '>=', Fey::Placeholder->new() )->group_by( $page_t->columns() )
        ->order_by( $count, 'DESC', $page_t->column('title'), 'ASC', );
...
sub _MemberCountSelect {
    ...
    my $file_count_select = Milki::Schema->SQLFactoryClass()->new_select();
    $file_count_select->select($count)->from($uwr_t)
...
sub _FileCountSelect {
    ...
    my $file_count_select = Milki::Schema->SQLFactoryClass()->new_select();
    $file_count_select->select($count)->from($file_t)
...
sub _BuildFilesSelect {
    ...
    my $files_select = Milki::Schema->SQLFactoryClass()->new_select();
    $files_select->select($file_t)->from($file_t)
        ->where( $file_t->column('wiki_id'), '=', Fey::Placeholder->new() )
...
sub members {
    ...
    return Fey::Object::Iterator::FromSelect->new(
        classes => ['Milki::Schema::User', 'Milki::Schema::Role'],
        select  => $select,
        dbh => Milki::Schema->DBIManager()->source_for_sql($select)->dbh(),
        bind_params => [$self->wiki_id()],
    );
...
sub _BuildMembersSelect {
    ...
    my $members_select = Milki::Schema->SQLFactoryClass()->new_select();
    $members_select->select( $user_t, $role_t )->from( $user_t, $uwr_t )
        ->from( $uwr_t, $role_t )
        ->where( $uwr_t->column('wiki_id'), '=', Fey::Placeholder->new() )
        ->order_by(
        $role_t->column('name'),          'ASC',
...
sub _BuildActiveUsersSelect {
    ...
    my $users_select = Milki::Schema->SQLFactoryClass()
        ->new_select( auto_placeholders => 0 );
    $users_select->select( $user_t, $order_by )->distinct()
        ->from( $user_t, $page_revision_t )->from( $page_revision_t, $page_t )
        ->where( $page_t->column('wiki_id'), '=', Fey::Placeholder->new() )
...
sub text_search {
    ...
    my $rank
        = Fey::Literal::Function->new( 'TS_RANK', $pst_t->column('ts_text'),
        $query, );

    $rank->set_alias_name('rank');

    my $search_select = Milki::Schema->SQLFactoryClass()->new_select();

    $search_select->select( $page_t, $page_revision_t, $rank )
        ->from( $page_t, $page_revision_t )->from( $page_t, $pst_t )
        ->where( $pst_t->column('ts_text'), '@@', Fey::Placeholder->new() )
        ->and( $page_t->column('wiki_id'), '=', Fey::Placeholder->new() )
        ->and( $page_revision_t->column('revision_number'),
        '=', $max_revision )
        ->order_by( $rank, 'DESC', $page_t->column('title'), 'ASC' );
    $search_select->limit( $limit, $offset );

    my $select = Milki::Schema->SQLFactoryClass()->new_select();

    my $marker = Milki::Schema::TextSearchResult->HighlightMarker();
    my $result = Fey::Literal::Function->new(
        'TS_HEADLINE',
        Fey::Literal::Term->new( q{title || E'\n' || content}, ),
        $query, "StartSel = $marker, StopSel = $marker",
    );
    ...
    $select->select( $star, $result )->from($search_select);
    ...
    for my $col_name ( sort map { $_->name() } $page_t->columns() ) {
        $attribute_map{ $x++ }
            = { class => 'Milki::Schema::Page', attribute => $col_name, };
    }

    for my $col_name ( sort map { $_->name() } $page_revision_t->columns() ) {
        $attribute_map{ $x++ } = {
            class     => 'Milki::Schema::PageRevision',
            attribute => $col_name,
        };
    }
    ...
    $attribute_map{ ++$x }
        = { class => 'Milki::Schema::TextSearchResult', attribute => 'result',
        };

    return Fey::Object::Iterator::FromSelect->new(
        classes => [
            'Milki::Schema::Page', 'Milki::Schema::PageRevision',
            'Milki::Schema::TextSearchResult'
        ],
        select => $select,
        dbh    => Milki::Schema->DBIManager()->source_for_sql($select)->dbh(),
...
sub pages_tagged {
    ...
    my $tag = $self->_find_tag_by_name($tag_name) or return;
...
sub _BuildPagesByTagSelect {
    ...
    $select->select( $page_t, $page_revision_t )
        ->from( $page_t, $page_revision_t )->from( $page_t, $page_tag_t )
        ->where( $page_tag_t->column('tag_id'), '=', Fey::Placeholder->new() )
        ->and( $page_revision_t->column('revision_number'),
        '=', $max_revision )->order_by( $page_t->column('title') );
...
sub pages_tagged_count {
    my $self = shift;
    my ($tag_name) = validated_list( \@_, tag => { isa => Str }, );

    my $tag = $self->_find_tag_by_name($tag_name) or return 0;
    ...
    my $vals
        = $dbh->selectrow_arrayref( $select->sql($dbh), {}, $tag->tag_id(), );
...
sub _BuildPagesByTagCountSelect {
    ...
    $select->select($count)->from($page_tag_t)
        ->where( $page_tag_t->column('tag_id'), '=', Fey::Placeholder->new() );
...
sub _find_tag_by_name {
    ...
    return Milki::Schema::Tag->new( tag => $name, wiki_id => $self->wiki_id(),
    );
}
...
sub _BuildPopularTagsSelect {
    ...
    $select->select( $tag_t, $count )->from( $tag_t, $page_tag_t )
        ->where( $tag_t->column('wiki_id'), '=', Fey::Placeholder->new() )
        ->group_by( $tag_t->columns() )
        ->order_by( $count, 'DESC', $tag_t->column('tag'), 'ASC' );
...
sub _TagCountSelect {
    ...
    my $distinct
        = Fey::Literal::Term->new( 'DISTINCT ', $page_tag_t->column('tag_id'),
        );

    my $count = Fey::Literal::Function->new( 'COUNT', $distinct );

    $select->select($count)->from( $page_t, $page_tag_t )
...
{
    my $guest = Milki::Schema::Role->Guest();
    my $read  = Milki::Schema::Permission->Read();

    my ( $wiki_t, $wrp_t ) = $Schema->tables( 'Wiki', 'WikiRolePermission' );

    my $base = Milki::Schema->SQLFactoryClass()->new_select();
...
sub _BuildAllWikiSelect {
    ...
    $select->select($wiki_t)->from($wiki_t)
        ->order_by( $wiki_t->column('title') );
...
sub _BuildMaxRevisionSelect {
    ...
    my $max_func
        = Fey::Literal::Function->new( 'MAX',
        $page_revision_t->column('revision_number'),
        );

    my $max_revision = Milki::Schema->SQLFactoryClass()->new_select();
